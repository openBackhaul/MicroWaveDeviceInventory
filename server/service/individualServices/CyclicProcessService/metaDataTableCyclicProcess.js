...
exports.MetaDataTableCyclicProcess = async function MetaDataTableCyclicProcess() {
  try {
    ...
    let unitForConnectionStatusSyncPeriod = profileInstanceForMetadataTableRetentionPeriod[onfAttributes.INTEGER_PROFILE.PAC][onfAttributes.INTEGER_PROFILE.CAPABILITY][onfAttributes.INTEGER_PROFILE.UNIT];
  ...
  try {
    ...
    if (metaDataListFromElasticSearch.length == 0) {
      ...
      */
      ...
      try {
        await individualServicesService.writeMetaDataListToElasticsearch(stringifiedMetaDataList);
      } catch (error) {
        console.log(error);
      }
    } else {
      ...
      for (let i = 0; i < metaDataListFromElasticSearch.length; i++) {
        ...
        for (let j = 0; j < odlDeviceList.length; j++) {
          if (metaDataListFromElasticSearch[i]['mountName'] == odlDeviceList[j]["node-id"]) {
            found = true;
            if (metaDataListFromElasticSearch[i]["connection-status"] !==
              odlDeviceList[j]["netconf-node-topology:connection-status"]) {
              if (metaDataListFromElasticSearch[i]["connection-status"] == "unable-to-connect" || 'connecting' && odlDeviceList[j]["netconf-node-topology:connection-status"] == "connected") {
                ...
                break;
              } else if (metaDataListFromElasticSearch[i]["connection-status"] == "connected" && odlDeviceList[j]["netconf-node-topology:connection-status"] == "unable-to-connect" || 'connecting') {
              ...
              if (metaDataListFromElasticSearch[i]["connection-status"] == "unable-to-connect" || "connecting") {
      ...
      console.log("----------updated metadataDeviceList------------", odlDeviceList, "-------------------------");
      let odlDeviceListString = JSON.stringify(odlDeviceList);
      await metaDataUtility.writeMetaDataListToElasticsearch(odlDeviceListString)
        .catch((error) => {
          throw error;
        })
    }
